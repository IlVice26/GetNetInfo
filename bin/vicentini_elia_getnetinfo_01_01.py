"""
Si vuole monitorare la configurazione (attributi che si ritengono significativi)
delle schede di rete installate sul computer locale e remoti accedendo alla
classe CIM, NetAdapterConfiguration. Il programma python getnetinfo.py avrà
il parametro enalbe in ingresso con cui decidere se monitorare le informazioni
delle schede abilitate (parametro -y) o se trattare le informazioni di tutte le
schede presenti (-n).
...
def main():
    ...
    if len(argv) < 2 or len(argv) > 3:              #Controllo degli argomenti
        print('Errore, richiesto solo un argomento')
    else:
